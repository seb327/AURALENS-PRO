...
| 4 | App Store Connect | Create in-app product `auralens_instant_reading_199` (Consumable, £1.99) | Already in `constants/products.ts` |
| 5 | App Store Connect | Create subscription `auralens_monthly_799` (Auto-Renewable, £7.99/month, group `auralens_main`) | Already in `constants/products.ts` |
...
| 10 | Google Play Console | Create in-app product `auralens_instant_reading_199` (Managed → Consumable) — set Active | Already in `constants/products.ts` |
| 11 | Google Play Console | Create subscription `auralens_monthly_799` with base plan `monthly` — set Active | Already in `constants/products.ts` |
...
| 20 | RevenueCat | Create entitlement `monthly` → attach `auralens_monthly_799` (iOS + Android) | Already referenced as `ENTITLEMENTS.monthly` in `constants/products.ts` |
